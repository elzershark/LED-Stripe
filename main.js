...
on({id: 'temperature', change: "any"}, function (obj) {
  var value = obj.state.val;
  var oldValue = obj.oldState.val;
  temperatur = getState("temperature").val;
  if (temperatur == 250 || temperatur == 182 || temperatur == 455 || temperatur == 370 || temperatur == 143) {
    setState("dimmer"/*dimmer_*/, Helligkeit);
  }
  if (temperatur == 250) {
    r1 = 255;
    g1 = 255;
...
Helligkeit = 100;
an_aus = 0;
on({id: 'hue', change: "any"}, function (obj) {
  var value = obj.state.val;
  var oldValue = obj.oldState.val;
  setState("dimmer"/*dimmer_*/, Helligkeit);
  h1 = getState("hue").val / 360;
  s1 = getState("saturation").val / 100;
  v1 = getState("dimmer").val / 100;
  i1 = Math.floor(h1 * 6);
  f1 = h1 * 6 - i1;
  p1 = (1 - s1) * v1;
  k1 = f1 * s1;
  ...
  timeout2 = setTimeout(function () {
    setState("color"/*Set default color of the lamp*/, ([Math.round(r1),',',Math.round(g1),',',Math.round(b1)].join('')));
  }, 500);
});

on({id: 'dimmer', change: "any"}, function (obj) {
  var value = obj.state.val;
  var oldValue = obj.oldState.val;
  Dimmer = getState("dimmer").val;
  timeout3 = setTimeout(function () {
    if (Dimmer == 100 && an_aus == 0) {
      an_aus = 1;
      setState("dimmer"/*dimmer_*/, Helligkeit);
      setState("mode"/*Set mode*/, 'all');
    } else if (Dimmer == 0) {
      an_aus = 0;
      setState("mode"/*Set mode*/, 'off');
    }
  }, 500);
});
on({id: 'dimmer', change: "gt"}, function (obj) {
  var value = obj.state.val;
  var oldValue = obj.oldState.val;
  Dimmer = getState("dimmer").val;
  timeout4 = setTimeout(function () {
    if (Dimmer < 100 && an_aus == 1 && Helligkeit != Dimmer) {
      Helligkeit = Dimmer;
      setState("brightness"/*Set brightness*/, ((Dimmer / 100) * 255 + 1));
    }
  }, 500);
});
on({id: 'dimmer', change: "lt"}, function (obj) {
  var value = obj.state.val;
  var oldValue = obj.oldState.val;
  Dimmer = getState("dimmer").val;
  timeout5 = setTimeout(function () {
    if (Dimmer > 0 && an_aus == 1 && Helligkeit != Dimmer) {
      Helligkeit = Dimmer;
      setState("brightness"/*Set brightness*/, ((Dimmer / 100) * 255));
    }
  }, 500);
});
